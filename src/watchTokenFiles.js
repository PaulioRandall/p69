import path from 'node:path'
import chokidar from 'chokidar'

import readTokenFiles from './readTokenFiles.js'
import util from './util.js'

export default function (tokenFiles, callback, options) {
	const chokidarOptions = prepChokidarOptions(options)

	if (!Array.isArray(tokenFiles)) {
		// User usually passes a file path rather than array of
		// paths. Put single it in an array makes things easier.
		tokenFiles = [tokenFiles]
	}

	const handler = (filePath) => {
		util.log(`Reloading token files\n`)
		const tokenMaps = readTokenFiles(tokenFiles)
		callback(tokenMaps)
	}

	const logFiles = '\n\t+ ' + tokenFiles.join('\n\t+ ')
	util.log(`Watching token files:${logFiles}`)

	return chokidar
		.watch(tokenFiles, chokidarOptions) //
		.on('ready', handler)
		.on('add', handler)
		.on('change', handler)
		.on('unlink', handler)
		.on('addDir', handler)
		.on('unlinkDir', handler)
		.on('error', (e) => {
			util.logError(`Watcher error: ${e}\n`) //
		})
}

function prepChokidarOptions(options) {
	const chokidarOptions = {
		// True to prevent recompile for each file under
		// src during start up.
		ignoreInitial: true,

		// Don't be silly.
		followSymlinks: false,

		// I don't know what is suitable but seems to work
		// fine. Extend 'stabilityThreshold' if you
		// experience file update issues.
		awaitWriteFinish: {
			// How long after a change is detected before
			// recompiling.
			stabilityThreshold: 500,
			pollInterval: 100,
		},

		// Avoid triggering recompile twice when a tool
		// deletes then writes the same file, rather than
		// modifying it.
		atomic: 200,
	}

	if (options.chokidar) {
		Object.assign(chokidarOptions, options.chokidar)
	}

	return chokidarOptions
}
